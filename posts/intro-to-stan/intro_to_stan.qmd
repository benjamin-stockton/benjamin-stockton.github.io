...
This post is the second workshop in the 2023-2024 Student Workshop Series on Computing at UConn. I'm re-publishing it here.
...
1.  **The Prior:** a distribution for the parameters that doesn't depend on the data.
    1.  For the regression model, we have three parameters $\beta_0, \beta_1, \sigma^2$. We assign a probability distribution $p(\beta_0, \beta_1, \sigma^2) \equiv p(\boldsymbol{\beta}, \sigma^2)$ to these parameters. Two common ways to do this are a non-informative prior so that the prior is uniform $p(\boldsymbol{\beta}, \log\sigma) = 1 \equiv p(\boldsymbol{\beta}, \sigma^2) \propto \sigma^{-2}$.
...
Stan is reporting large rhat values and divergent transitions, so we can also take a look at the traceplots of the samples in @fig-brm-mi-trace.
